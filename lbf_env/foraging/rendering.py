...
class Viewer(object):
    ...
    def _draw_players(self, env):
        ...
        for i, p in enumerate(env.players):
            ...
            self._draw_agent_nr(*p.position, i)

    def _draw_agent_nr(self, row, col, nr):
        badge_x = col * self.grid_size + (3 / 5) * self.grid_size
        badge_y = self.height - self.grid_size * (row + 1) + (3 / 4) * self.grid_size
