...
class Scenario(object):
    def __init__(self, config):
        ...
        try:
            scenario = importlib.import_module(
                "football_env.scenarios.{}".format(config["level"])
            )
        except ImportError as e:
            logging.error('Loading scenario "%s" failed, check import in scenario_builder.py' % config["level"])
            logging.error(e)
            sys.exit(1)
    ...
    def _BuildScenarioConfig(self):
        ...
        if not self._scenario_cfg.deterministic:
            # print("game_engine_random_seed", self._config['game_engine_random_seed'])
