...
from gfootball.env import constants, football_action_set, observation_rotation

from football_env.core import config as cfg
from football_env.core import football_env_core
...
class FootballEnv(gym.Env):
    ...
    def _construct_players(self, definitions, config):
        ...
        name=agent, d={'left_players': '11', 'right_players': '0'}
        name=bot, d={'left_players': '0', 'right_players': '1'}
        """
        result = []
        left_position = 0
        right_position = 0
    ...
    def _convert_observations(
        ...
        for is_left in [True, False]:
            adopted = (
                original
                if is_left or player.can_play_right()
            ...
            for x in range(
                ...
                if position + x >= len(adopted[prefix + "_agent_controlled_player"]):
                    ...
                    o["active"] = adopted[prefix + "_agent_controlled_player"][
                        position + x
                    ]
    ...
    def _get_actions(self):
        ...
        for player in self._players:
            adopted_obs = self._convert_observations(
                obs, player, left_player_position, right_player_position
            )
            ...
            assert len(adopted_obs) == len(
                a
            ), "Player provided {} actions instead of {}.".format(
                len(a), len(adopted_obs)
            )
            if not player.can_play_right():
                #True case for agent player in inference
    ...
    def step(self, action):
        """
        Here only agent actions are provided, 
        while in _get_actions(), also bot actions are taken. 
        All other actions of players, that are not agents nor bots, 
        are generated internally by the enginge (internal bots, controlled by difficulty level in scenario)
        """
        action = self._action_to_list(action)
        if self._agent:
            # we set action here and retrieve it in _get_action() back
            self._agent.set_action(action)
        else:
            assert (
                len(action) == 0
            ), "step() received {} actions, but no agent is playing.".format(
                len(action)
            )
