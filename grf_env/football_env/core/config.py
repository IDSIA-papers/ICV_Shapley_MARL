...
def count_players(definition):
    ...
    return int(player_definition["left_players"]) + int(
        player_definition["right_players"]
    )
...
def get_agent_number_of_players(players):
    ...
    return sum(
        [count_players(player) for player in players if player.startswith("agent")]
    )
...
class Config(object):
    ...
    def NewScenario(self, inc=1):
        ...
        from football_env.core import scenario_builder
