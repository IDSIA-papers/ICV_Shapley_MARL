...
class FootballEnvCore(object):
    ...
    def _reset(self, animations, inc):
        ...
        assert (
            self._env.state == GameState.game_created
            or self._env.state == GameState.game_running
        ...
        assert (
            not scenario_config.dynamic_player_selection
            or not scenario_config.control_all_players
        ), (
            "For this scenario you need to control either 0 or all players on the "
            "team ({} for left team, {} for right team)."
        ).format(
    ...
    def step(self, action, extra_data={}):
        ...
        for left_team in [True, False]:
            agents = (
                self._env.config.left_agents
                if left_team
                else self._env.config.right_agents
            )
            for i in range(agents):
                ...
                if self._env.waiting_for_game_count == 20:
                    ...
                    controlled_players = (
                        ...
                        if left_team
                        else self._observation["right_agent_controlled_player"]
                    )
                    if (
                        self._observation["ball_owned_team"] != -1
                        and self._observation["ball_owned_team"] ^ left_team
                        and controlled_players[i]
        ...
        if (
            self._env.config.end_episode_on_out_of_play
            and self._observation["game_mode"]
            != int(libgame.e_GameMode.e_GameMode_Normal)
            and self._state.previous_game_mode
            == int(libgame.e_GameMode.e_GameMode_Normal)
        ):
            self._env.state = GameState.game_done
        ...
        if (
            self._env.config.end_episode_on_possession_change
            and self._observation["ball_owned_team"] != -1
            and self._state.prev_ball_owned_team != -1
            and self._observation["ball_owned_team"] != self._state.prev_ball_owned_team
        ):
            self._env.state = GameState.game_done
        ...
        debug["reward"] = reward
        if self._observation["game_mode"] != int(libgame.e_GameMode.e_GameMode_Normal):
        ...
        if episode_done:
            ...
            logging.info(
                ...
                single_observation["score"][0],
                single_observation["score"][1],
                self._step_count,
                fps,
                game_fps,
            )
